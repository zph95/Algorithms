...
int Sort[N + 1] = {
    N}; // 存储按u.f从大到小排序时的点，按点的顺序调用DFS_Visit函数
  ...
  for (i = 1; i <= N; i++) {
    ...
    if (G->V[j].color == WHITE) {
      if (flag)
        dataout << "强连通分量为：( ";
      DFS_Visit(G, &G->V[j]);
      // flag == 1时，第二次调用DFS，此时每次调用DFS_Visit
      // 就会输出一个强连通分量
      if (flag)
        dataout << ")" << endl;
    }
...
string filename1, filename3;
int main(int, char **) {
  std::cout << "Hello, from scc_src_test!\n";
  // 构造一个空的图
  Graph *G = new Graph;
  int edge[N * LOGN][2]; // 边数组
  srand((unsigned)time(NULL));

  filename1 = "input/size1/input.txt";
  filename3 = "output/size1/output1.txt";
  ...
  std::cout << "End from cmake_unit_test!\n";
  return 0;
}
