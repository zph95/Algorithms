...
void RB_INSERT_INSERT_FIXUP(struct Tree *z) {
  while (z->parent->color == RED) {
    if (z->parent == z->parent->parent->left) {
      ...
      if (y->color == RED) { // 解决了z的父结点和z都是红色结点问题
        ...
        z->parent->color =
            BLACK; // case3：是z是一个左孩子且叔结点为黑色，改变z的父和祖父结点颜色并做一次右旋，以保持性质5
        z->parent->parent->color = RED;
...
void RIGHT_ROTATE(struct Tree *x) { // 右旋转
...
void LEFT_ROTATE(struct Tree *x) { // 左旋转
...
struct Tree *
TREE_MINIMUM(struct Tree *x) { // 非递归版本的查找二叉查找树的最小值
  while (x->left != nil) {
    x = x->left;
  }
  return x;
...
void RB_DELETE_FIXUP(struct Tree *x) {
  ...
  while (x != root &&
         x->color == BLACK) // 如果x是黑色并且不是根结点，才进行循环。
  { // x是一个具有双重颜色的结点，调整的目的是把x的黑色属性向上移动。
    if (x == x->parent->left) {
      ...
      if (w->left->color == BLACK &&
          w->right->color ==
              BLACK) // 情况二：x的兄弟结点w是黑色的，而且w的两个子节点都是黑色。
      {
        w->color = RED; // 从x和w上去掉一重黑色。x还是黑色，而w变为红色。
        x = x->parent; // x结点向上移动成为新的待调整结点。
      } else {
        if (w->right->color ==
            BLACK) // 情况三：x的兄弟结点w是黑色的，w的左孩子是红色的，w的右孩子是黑色的。
        ...
        w->color =
            x->parent
                ->color; // 以下是情况四：x的兄弟结点w是黑色的，且w的右孩子是红色的。
        x->parent->color =
            BLACK; // 置x.p和w.right为黑色+旋转使其去掉x的额外黑色。
        w->right->color = BLACK;
        LEFT_ROTATE(x->parent);
        x = root; // x成为根结点，结束循环。
      }
...
void RB_DELETE(struct Tree *z) {
  ...
  if (z->left == nil) {
    ...
    y->left->parent = y;
    y->color =
        z->color; // 把已经删除的结点颜色赋值给y，保证了y以上的树结构红黑性质不变。
  }
...
int main(int, char **) {
  std::cout << "Hello, from cmake_unit_cpp!\n";
  int i, j, k, num;
  int *b;
  int *a;
  a = new int[MAX];
  ...
  for (i = 0; i < MAX; i++)
    a[i] = i + 1; // 将数组a中数字置1~36
  for (i = 0; i < MAX; i++) {
    j = rand() % MAX;
    // 将a[i]与a[j]交换，a[j]为任意一个数 num = a[j]; a[j] = a[i]; a[i] = num;
  }
  for (i = 0; i < N; i++) {
    out << a[i] << endl;
  }
  ...
  filename =
      "input_numble.txt"; // 打开其它文件夹内的文件./为当前层，../为上层
  ...
  for (n = 20; n <= N; n += 20) {
    ...
    nil->key = Nil;
    nil->color = BLACK; // 设置nil结点
    nil->size = 0;

    filename1 = "output_size";
    ...
    filename2 = filename1 + "_preorder.txt"; // 遍历红黑树
    dataout.open(filename2);
    preorderTraverse(ROOT[k]);
    dataout.close();
    filename2 = filename1 + "_inorder.txt";
    dataout.open(filename2);
    InorderTraverse(ROOT[k]);
    dataout.close();
    filename2 = filename1 + "_posteorder.txt";
    dataout.open(filename2);
    ...
    filename2 = filename1 + "_delete_data.txt";
    dataout.open(filename2);
    ...
    filename2 = filename1 + "_delete_inorder.txt"; // 遍历删除后的序列
    dataout.open(filename2);
    InorderTraverse(ROOT[k]);
    dataout.close();
    k++;
